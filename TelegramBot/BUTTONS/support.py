...
import re
from ..states import Support, UserMenu, OrderCommercial
...
@dp.callback_query_handler(text='write_admin', state=[Support.main_menu, OrderCommercial.billing])
async def my_orders(cq: CallbackQuery, state: FSMContext):
    msg = cq.message
    user_id = msg.chat.id
    await bot.edit_message_text(chat_id=user_id, message_id=msg.message_id, text=f'Пиши сюда: {cfg.admin_tag}', reply_markup=kbd.single_menu_btn())
